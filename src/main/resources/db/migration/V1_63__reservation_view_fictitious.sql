-- Fictitious admin-only replacement reservations have `reservation_flow.offer_id = NULL`
-- (no partner offer row exists — agency handled the swap in Nausys/MMK).
-- Regenerate reservation_view so those rows still surface in admin + customer
-- lists: join yacht directly via reservation_flow.yacht_id, LEFT JOIN offer,
-- COALESCE location lookups between offer and reservation fields.
