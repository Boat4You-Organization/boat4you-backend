-- MMK can return long Terms of Payment / remarks; lift VARCHAR caps to 2000.
-- reservation_view depends on payment_note + note, so drop & recreate (V1_63 def).
DROP VIEW IF EXISTS reservation_view;

ALTER TABLE reservation ALTER COLUMN payment_note TYPE VARCHAR(2000);
ALTER TABLE reservation ALTER COLUMN note TYPE VARCHAR(2000);
