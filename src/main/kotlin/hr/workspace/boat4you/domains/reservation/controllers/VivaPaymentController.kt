...
@Validated
@Controller
@RequestMapping("/secured/payments/viva")
@PreAuthorize("isAuthenticated()")
class VivaPaymentController(
    private val paymentService: VivaPaymentService,
) {
    @Operation(summary = "Create a Viva checkout session")
    ...
    ): ResponseEntity<VivaInitiateResponseDto> {
        val vivaResponse = paymentService.initiatePayment(reservationId, payFullAmount, paymentPhaseId)
        return ResponseEntity.ok(vivaResponse)
    }

    @Operation(summary = "Check payment status for Viva", description = "Used when performing card payment via Viva, after redirect to the success URL")
    @PostMapping("/checkPaymentStatus/{vivaOrderCode}")
    fun checkPaymentStatus(
        @PathVariable vivaOrderCode: String,
    ): ResponseEntity<CheckoutSessionDto> {
        return ResponseEntity.ok(paymentService.checkPaymentStatus(vivaOrderCode))
    }
}
