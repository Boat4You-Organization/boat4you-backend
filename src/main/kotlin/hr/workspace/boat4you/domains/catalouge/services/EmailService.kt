...
class EmailService(
    ...
    fun sendEmail(
        ...
        /** Raw MIME headers set on the message (e.g. List-Unsubscribe /
         *  List-Unsubscribe-Post for campaign mail). Applied after the
         *  helper fills the standard fields. */
        extraHeaders: Map<String, String> = emptyMap(),
        /** Send on the caller's thread and rethrow MailException instead of
         *  the default async fire-and-forget. Campaign sending needs the
         *  real per-recipient SMTP outcome to mark SENT vs FAILED. Only
         *  meaningful outside a JPA transaction. */
        syncSend: Boolean = false,
    ) {
        ...
        // Raw headers last — after the helper, so nothing overwrites them.
        extraHeaders.forEach { (name, value) ->
            message.setHeader(name, value)
        }

        if (syncSend) {
            // Campaign path: caller needs the real SMTP outcome per message;
            // MailException propagates so the job can mark FAILED.
            logger.debug("Sending email (sync) with subject '{}' to '{}'", subject, recipients)
