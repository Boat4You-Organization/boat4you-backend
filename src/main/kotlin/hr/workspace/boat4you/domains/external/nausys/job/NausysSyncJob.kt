...
class NausysSyncJob(
    ...
    // PT4H (was PT2H): the offer sync now also queries the 7-day Sat->Sat grid up
    // to 18 months, so a full run is longer — give the lock headroom so it never
    // releases mid-run (it still finishes well before the 06:15 backup pass).
    @SchedulerLock(name = "nausysYachtSync", lockAtMostFor = "PT4H")
    fun runYachtSync() {
